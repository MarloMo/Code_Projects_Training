...
#     """
#     The Manhattan heuristic calculates the sum of the x and y
#     distances together. For graphs that allow diagonal movement, the
#     Euclidean heuristic is a better choice
#     """
...
def heuristic(start, target):
    x_distance = abs(start.position[0] - target.position[0])
    y_distance = abs(start.position[1] - target.position[1])
    return np.abs(x_distance * x_distance + y_distance * y_distance)
